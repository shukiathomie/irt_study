...
class Heu_Algo:
    ...
    def Parallel_step1(self, j, X):  # モデルの作成
        opt_x = Opt_x(self.U, X, self.T)
        opt_x.modeling(j=j)
        # モデルの最適化
        x_opt, obj = opt_x.solve()
        return x_opt, obj

    def Parallel_step2(self, i, Y):
        # モデルの作成
        opt_y = Opt_y(self.U, Y, self.T)
        opt_y.modeling(i=i)
        # モデルの最適化
    ...
    def repeat_process(self, Y):
        ...
        with LoggerUtil.tqdm_joblib(self.I):
            out = Parallel(n_jobs=-1, verbose=0)(
                delayed(Heu_Algo.Parallel_step2)(self, i, X_opt)
                for i in range(self.I)
            )
        # self.logger.info(f"{out}")
    ...
    def heu_algo(self):
        # step0 init_Y
        self.logger.info(f"first step")
        best_Y = self.init_Y
        X_opt, Y_opt = Heu_Algo.repeat_process(self, best_Y)
        i = 1
